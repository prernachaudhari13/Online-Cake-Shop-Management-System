...
if($_POST[randomid] == $_SESSION[randomid])
{
	if(isset($_POST[submit]))
	{
		if(isset($_GET[editid]))
		{
			...
			$sql ="UPDATE user SET user_type='$_POST[usertype]',login_id=$_POST[loginid]',";
			if($_POST[password] != "")
			{
			$sql = $sql . "password='$pwd',";
			}
			$sql = $sql . "name='$_POST[name]',mob_no='$_POST[phone]',address='$_POST[address]',status='$_POST[status]' WHERE user_id='$_GET[editid]'";
		...
		{
			$pwd = md5($_POST[password]);
		 $sql ="INSERT INTO user (user_type,login_id,password,name,mob_no,address,status) values('$_POST[usertype]','$_POST[loginid]','$pwd','$_POST[name]','$_POST[phone]','$_POST[address]','$_POST[status]')";
		 $qsql = mysqli_query($con,$sql);
			if(!$qsql)
			{
				echo mysqli_error($con);
...
function validateform()
{
	...
	document.getElementById("idpswd").innerHTML ="";
	document.getElementById("idcpswd").innerHTML  ="";
	...
	{
		document.getElementById("idpswd").innerHTML ="<font color='red'>Password should be atleast 6 characters. </font>";
		errmsg=1;
	}	
	if(document.frmuser.password.value =="")
	{
		document.getElementById("idpswd").innerHTML ="<font color='red'>Please enter the password.</font>";
		errmsg=1;
	}
	if(document.frmuser.cpassword.value =="")
	{
		document.getElementById("idcpswd").innerHTML ="<font color='red'>Please confirm your password.</font>";
		errmsg=1;
	}
	if(document.frmuser.password.value != document.frmuser.cpassword.value)
	{
		document.getElementById("idcpswd").innerHTML ="<font color='red'>Password and confirm password are not matching.</font>";
		errmsg=1;
	}
	if(!document.frmuser.name.value.match(alphaspaceExp))
	{
		document.getElementById("idnm").innerHTML ="<font color='red'>Enter only alphabets.</font>";
		errmsg=1;
	}
